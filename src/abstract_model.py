...
from pytorch_tabnet import tab_network, disc_network
...
class TabModel(BaseEstimator):
    ...
    def fit(
        ...
        self.n_target =y_train.shape[1]
        self.discriminator=disc_network.Discriminator(self.n_features+self.n_target,self.n_target).to(self.device)
        self.generatorLoss=[]
        self.discriminatorLoss=[]
        self.criterion =torch.nn.BCELoss()
        self.disc_opt = torch.optim.SGD(self.discriminator.parameters(), lr=0.1)
        #
    ...
    def _train_epoch(self, train_loader):
        ...
        for batch_idx, (X, y) in enumerate(train_loader):
            ...
            #edit
            self.epoch_loss_disc = []
            self.epoch_loss_gen =[]
            #
            batch_logs = self._train_batch(X, y)

            self._callback_container.on_batch_end(batch_idx, batch_logs)
            self.discriminatorLoss.append(sum(self.epoch_loss_disc)/len(self.epoch_loss_disc))
            self.generatorLoss.append(sum(self.epoch_loss_gen)/len(self.epoch_loss_gen))
    ...
    def _train_batch(self, X, y):
        ...
        #edit
        #Create the labels  
        real_labels = torch.ones(batch_logs["batch_size"],self.n_target).to(self.device)
        fake_labels = torch.zeros(batch_logs["batch_size"],self.n_target).to(self.device)
        ...
        inputs_real = torch.cat((X,y),dim=1).to(self.device)
        disc_real_pred = self.discriminator(inputs_real)
        disc_real_loss = self.criterion(disc_real_pred,real_labels)

        #Get discriminator loss for fake data
        z= np.random.normal(0,1,size=(batch_logs["batch_size"],self.n_target))
        z = torch.from_numpy(z).to(self.device)
        gen_input = torch.tensor(torch.cat((X,z),dim=1)).to(self.device) 
        gen_output, M_loss = self.network(gen_input)
        #print(generated_y.shape)
        inputs_fake = torch.cat((X,gen_output),dim=1).to(self.device) 

        disc_fake_pred = self.discriminator(inputs_fake) 
        disc_fake_loss = self.criterion(disc_fake_pred,fake_labels)

        #Get the discriminator loss 
        disc_loss = (disc_fake_loss + disc_real_loss) / 2
        #print(disc_loss)
        self.epoch_loss_disc.append(disc_loss.item())
        ...
        z= np.random.normal(0,1,size=(batch_logs["batch_size"],self.n_target))
        z = torch.from_numpy(z).to(self.device)
        gen_input = torch.cat((X,z),dim=1).to(self.device) 
        #Generate input to generator using ABC pre-generator 
        gen_output, M_loss = self.network(gen_input)
        inputs_fake = torch.cat((X,gen_output),dim=1).to(self.device) 
        disc_fake_pred = self.discriminator(inputs_fake)
        loss = self.compute_loss(disc_fake_pred, real_labels)
        self.epoch_loss_gen.append(loss)
